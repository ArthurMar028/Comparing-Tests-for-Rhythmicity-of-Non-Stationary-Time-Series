...
#0. Data loader: Algoritmo de leitura de dados
...
plot(time[-1], (diff(posicao)/diff(time)[1])*0.1084, type="l", col="blue3")
...
par(mfrow=c(1,1))
       ...
       Autocorrel <- function(time = time, signal = diff(posicao), colour = "red3"){
         ...
         plot(autocorrel,
              main= NULL,
              ylab= "Função de Autocorrelação",
              xlab= "Deslocamento de fase",
              col = colour,
              sub = paste("O período principal segundo a função de autocorrelação é",
       ...
       ISI <- function(time = time, signal = diff(posicao), colour = "red3"){
         ...
         if(length(time)!=length(signal)){
           ...
           plot(time[-1], signal,
                ylab=expression(paste("Taxa de Crescimento (", mu, " ", min^-1, ")")), 
                xlab = "Tempo (min)",
                type="l", col=colour,
         ...
         abline(v=time_peaks[1:15], col="purple", lwd=0.01) #It's not exactly in the peaks
       ...
       Plot_growthvstime <- function(time = time, signal = posicao, colour="red3", median = TRUE, fitted = FALSE, micron_per_pixel = 1){
         
         growth_rate <- diff(signal)/mean(diff(time))*micron_per_pixel #(pixel/min)*(micron/pixel)
         
         title <- bquote("A média e mediana da taxa de crescimento instantânea são, respectivamente," ~ 
                            .(round(mean(growth_rate), digits = 3)) ~ "e" ~ 
                            .(round(median(growth_rate), digits = 3)) ~ mu ~ min^-1)
         
         subtitle <- bquote("A taxa de crescimento instantânea mínima é, respectivamente, de" ~ 
                              .(round(min(growth_rate), digits = 3)) ~ "e" ~ 
                              .(round(max(growth_rate), digits = 3)) ~ mu ~ min^-1)
         
         if(median==TRUE & fitted==FALSE){
           plot(time[-1], growth_rate,
                type="l", col=colour,
                ylab = expression(paste("Taxa de Crescimento Instantânea (  ", mu, " ", min^-1, ")")), xlab = "Tempo (min)",
                main = title,
                sub = subtitle,
       ...
       #Final plot - to be changed (this way of making multiple plots isn't really clear)
       ...
       mtext("Retirada tendência a partir de fit linear", side = 3, line = 1, adj = 0.5, cex = 1.2, font = 1)
       Plot_growthvstime(time, quad_model$residuals, colour = "green4", median = FALSE, fitted = TRUE)
       mtext("Retirada tendência a partir de fit quadrática", side = 3, line = 1, adj = 0.5, cex = 1.2, font = 1)
       
       # Plot Transformadas de Fourier
       Fourier_transform(time, diff(posicao))
       Fourier_transform(time, diff(linear_model$residuals), colour = "blue3")
       Fourier_transform(time, diff(quad_model$residuals), colour = "green4")
       
       # Plot acf
       Autocorrel(time, diff(posicao))
       Autocorrel(time, diff(linear_model$residuals), colour = "blue3")
       Autocorrel(time, diff(quad_model$residuals), colour = "green4")
       
       
       # Plot ISI
       ISI(time, diff(posicao))
       ISI(time, diff(linear_model$residuals), colour = "blue3")
       ISI(time, diff(quad_model$residuals), colour = "green4")
       
       par(mfrow = c(1, 1))
       
       #Wavelet - a editar e entender cada parâmetro
       
       
       > my.data <- data.frame(time = tempo[-1], signal = diff(posicao_um))
       
       > my.w <- analyze.wavelet(my.data, 
                                 +                         "signal",  # Name of the signal column
                                 +                         loess.span = 0.2,
                                 +                         dt = 4/60,  # Time step in minutes
                                 +                         lowerPeriod = 0.2,  # Minimum period in minutes
                                 +                         upperPeriod = 6,    # Maximum period in minutes  
                                 +                         make.pval = TRUE, 
                                 +                         n.sim = 100)  # Increased simulations for better p-values
       
       > wt.image(my.w, 
                  +          main = "Continuous Wavelet Transform",
                  +          timelab = "Tempo (min)",
                  +          periodlab = "Período (min)")
